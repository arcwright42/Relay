...
工作目录不是系统沙箱边界；工具执行权限由 Codex 的模式、沙箱及审批共同落实。Relay 不将预览资料库中的示例内容发送给 Agent。图片、文件附件、语音、划词入口及多 Agent 委派暂未接通。
...
`conversation.json` 保存可见消息、工具摘要、工作目录、来源、已确认配置，以及原生 session ID 引用。文件带格式版本，使用 0600 权限和临时文件替换；流式内容约每秒检查点，轮次结束与正常退出时刷新。恢复后将未结束的回复标记为 interrupted。无法解析或版本不支持的文件保留原样，禁止写入覆盖。
...
再次连接优先执行原生 `session/load`，不重复插入 Agent 重播的历史。无法复用时创建新会话，将 Relay 保存的近期可见对话作为历史上下文，最多 24,000 字符；这不是完整项目检索或长期记忆。隐藏推理与 Agent 内部缓存不迁移。输入草稿尚未持久化，项目列表仍是当前三个预览项目。
...
`codex_probe` 是主动执行的真实连接检查，使用测试项目 9001。默认仅安装、连接并读取模型数量，不调用推理；`--prompt` 才发送真实消息并产生正常模型用量。会复用现有 Codex 认证；没有登录时退出并提示从应用登录。必要时设置独立 `RELAY_DATA_DIR`，组件也会安装到该目录。测试项目不是产品项目列表的一部分。
