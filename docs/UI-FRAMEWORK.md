...
日期：2026-09-22。状态：采用 GPUI + GPUI Kit，已编译并运行原生工作台壳子；划词、语音与长对话等专项验证尚未开展。
...
当前锁定 GPUI Kit 0.6.6，并保存配套 GPUI 的 lockfile。工作台先保留左侧导航与中间工作区，按最新要求暂时移除右侧任务栏。开发构建提供原生 GPUI Inspector，可通过右键“检查元素”或 `⌘⌥I` 打开，进入选取模式后点击目标查看布局与样式。后续逐项完成上述关键交互验证与功能接入。
